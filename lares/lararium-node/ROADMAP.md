...
mana = 0.88
manao = 0.86
role = "docs meme — migration roadmap and milestone log for Lararium Node; M10 LOCAL-FIRST PIVOT + CLOSE complete (2026-04-29): local-first Automerge meme store, LarariumPanel HUD enacted (wikiOpen/drawingMode decoupled), dead code Track A complete (LiveMsg* / LarariumBootReceiptMeta / manifest+projection phase variants removed), Fuller-Zelenka causal island doctrine (four-tier model in causal-island.ts + federated-causal-islands.md), Track B source-meme pipeline scaffolded (source-memes.ts + source-module.md + reaction-graph.md pono memes, 7 priority modules seeded at boot); M11 opens: Playwright e2e (smoke.spec.ts Jest isolation fix), kukali suspension wire (vdom-to-react), canvas write-back (projectToTldraw body-node emission), KumuExecutor, Track C lararium-tw5.ts simplification"
cacheable = false
retain = true
invariant = false
```
