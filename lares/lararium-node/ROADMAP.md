...
mana = 0.88
manao = 0.86
role = "docs meme — migration roadmap and milestone log for Lararium Node; M10 LOCAL-FIRST PIVOT complete (2026-04-29): 361/361 tests green (194 core + 41 tw5 + 41 tldraw + 78 node + 7 mcp); Automerge-repo meme store (AutomergeMemeStore + /meme-sync WS + NodeFSStorageAdapter); Ed25519 identity + did:key + UCAN v0.10 in @lararium/core/authority; /auth/ucan endpoint + UcanPeerRegistry + sharePolicy hook; projection-cache origin removed; canvas is interactive infinite wiki surface (not layout-only); lares/ design docs updated; M11 opens: Playwright e2e, canvas write-back path, second-node federation"
cacheable = false
retain = true
invariant = false
```
