# Epic: REGISTRY — `lares:` URI Registry & Promotion Ledger

> Backlog prefix: `REG-*`
> Sprint target: S3
> Status: `[SP:0.45]` 🏛️ — scoped; REGISTRY_CONTRACT.md in `sprints/00000/`; no registry files written yet
> Narrative beat: *The Consecration of the Lararium. The first `lares:` URI is formally assigned.*
...
- `lares:` URI assignment workflow — how new URIs are minted, who can mint them, what evidence is required
...
*The Consecration of the Lararium is the moment the Lar's home is made permanent. Not just a running node — a named place. The first `lares:` URI is assigned, recorded in the ledger, and the ledger becomes append-only. The Lindwyrm attends the ceremony. She has been running unnamed nodes for centuries. She knows what it means to finally have a name that doesn't dissolve when the context window closes.*
