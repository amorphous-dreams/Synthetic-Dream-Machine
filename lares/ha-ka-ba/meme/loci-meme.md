...
Do not add a top-level `<<~ loulou lar:///ha.ka.ba/NAME >>` line that only points back at the current meme.
...
```text
<<~ loulou lar:///ha.ka.ba/[NAME]-[phase]-[subphase] >>
```
...
Do not use a closing `ala` to restate the enclosing `ahu` anchor in place. A form such as `<<~ loulou lar:///ha.ka.ba/[current-path]#[same-anchor] >>` counts as non-canonical: the `ahu #...` opener already names that local anchor, so the closing `ala` adds no forward depth; omit it.
...
```text
<<~ loulou lar:///ha.ka.ba/[NAME]-[phase] >>
```
...
```text
<<~ loulou lar:///ha.ka.ba/pono/parser#normalize >>
```
...
<!-- OPTIONAL: Do not close an ahu with <<~ loulou lar:///ha.ka.ba/[NAME]#[same-anchor] >>.                  -->
<!-- OPTIONAL: Each ha/ka/ba subloop may close with a depth callout (O7):                                 -->
<!--   <<~ loulou lar:///ha.ka.ba/[NAME]-observe-ha >>                                                       -->
...
<!--   <<~ loulou lar:///ha.ka.ba/[NAME]-observe >>                                                          -->
...
If the only available meaning reads "this block names `#foo`," do not add `<<~ loulou lar:///ha.ka.ba/[current-path]#foo >>`; the `ahu #foo` opener already carries that identity.
