...
The **implementors interface** it queries IS a meme: `lar:///ha.ka.ba/api/v0.1/lararium/modules/tw5-module-interface`. Any meme that declares `implements = "lar:///ha.ka.ba/api/v0.1/lararium/modules/tw5-module-interface"` with content-type `application/javascript` and passing signal fields will be injected by the corpus path.
