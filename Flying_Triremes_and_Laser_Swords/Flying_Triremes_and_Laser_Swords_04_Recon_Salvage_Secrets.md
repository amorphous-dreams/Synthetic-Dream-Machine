...
UVG defines **Discoveries** as interesting locations near Destinations, and it encourages map notes as a group asset. Players may note RSS nodes on maps next to Destinations or Discoveries, track them in their company journals, and buy/sell/trade the intel. The Attention clock then tells you how long you can push before the place pushes back.

- Each Discovery may contain **Nodes** (things you can exploit).
- Each Node carries **Tags** (what it yields, what it triggers).
- Node types influence the tags and resources usually found there:
  - **[Static]** resources from the world body; matter as form, hopefully renewable.
  - **[Active]** resources from the world soul; power as mana, leyline networks.
  - **[Dynamic]** resources from the world psyche; information as change, dream-code.

The referee may choose between 0-3 nodes based on the Area level, or roll a d6:
...
You may also roll a d6 to choose a node's type:
 
1. Edible/Medicinal Herbs or Raw Resources; [Static] tags.
2. Edible Fauna or Other Consumable; [Static] tags.
3. Magitech Salvage or Fantascience Components; [Active] tags.
4. Oldtech Battery or Leyline Node; [Active] tags.
5. Dream-Shard or Arcane Secret; [Dynamic] tags.
6. Ritual Site or Archive Uplink; [Dynamic] tags.
...
Attention measures how close the Area is to noticing you *as a problem*. Attention can mean creatures, guards, spirits, systems, guilds, wards, reputation, paperwork, Intrusion Countermeasure Electromagitech (ICE), or “the ruin wakes up.”

#### Attention Clock (0–19+)
...
- **20+** Emergency. The Area answers you *right now*.

These numbers do not model “morality.” They model **alarum**.

Run **one Attention score** per Area by default. "Area" could be a town, landmark location, "dungeon" level or sub-level, etc. Split into sub-clocks only when:
...
#### Attention Tick Size (escalation rate)
...
Tick size never drops below **+1**. Nothing stays invisible forever. Not in the Vastlands. Not on Elyncia.
...
Track Attention using a stack of token, an oversized d20, or some other player facing flag. If an action feels like it should increase Attention, it probably should.

Note the area Attention score(s) when the players leave an area.

#### Attention Breakpoints

At low levels, the Area reacts softly. At high levels, the Area reacts immediately.

#### 1–6: Quiet / Warm (soft pressure)

At **Attention 1+**, bring out the **OSR Encounter Die**.
...
- an ICE daemon starts watching the leyline.
...
#### Optional: Overloaded Encounter Die (Vastlands)

Some tables like a single die that eats resources and spawns encounters at once. Vastlands calls this **Overloaded Encounter Dice** and frames it as an option for exploration periods.
...
Attention rises fast because extraction leaves scars. Cooling down takes longer because the world has memory. Patrol routes, guild logs, ward-nets, gossip, and hungry spirits don’t reset just because you changed scenes. When the crew stops pushing the Area, Attention does start to drain. How fast it drains depends on the time scale you’re playing at.
...
#### Cooling Down by Time Scale

- **Turn (10 minutes):**  
  If the party spends a turn *not* doing RSS-facing work (no prying, no extraction, no hacking, no rituals), they may attempt to *stay quiet*. If they also take cover, reduce noise, and control light, the referee may allow a small drop in Attention (usually **–1**) once per several turns, not every turn.  *Rule of thumb:* you can hide from eyes in minutes, but you can’t erase consequences in minutes.

- **Watch (4 hours):**  
  A full watch spent *laying low* (Hide, misdirection, decoys, bribery, paperwork, changing routes, letting the ruin “forget”) reduces Attention by **–1d4** (minimum **–1**).  
  A watch spent *resting* reduces Attention by **–1** if you truly keep your head down (no fireworks, no rites, no loud heroics).

- **Week:**  
  Leaving the Area and not returning for a week reduces Attention by **–1d6** (minimum **–1**) as rumors cool, patrol rotations change, and the system’s “recent events” log rolls over.  
  If you return immediately, you return to the same heat. The place remembers you *this week*.

#### Ways to Cool Down

Players should be able to *buy down heat* with choices, not just math:
...
More examples in FTLS Equipment document (in progress):

\~/documents/FTLS/drafts/Equipment
...
Active (ka)    Mana spores, living flux crystals, chaos motes, orgone energy   Stabilized mana (2d6\* mana/power, 1 st, \[dangerous\] if handler is Misaligned)  
...
Standard rate: 1 refined unit ≈ 100 € or 1 Bonus/Ticket Die trinket. Adjust by region & rarity.
