...
**Added**
- `lares handle publish "<glamour>"` — the first surface that publishes a persona's public "here I am" note (a Handle) and the first production caller of `publishPersonaGlamour`. It reads the persona-KEL prefix off the daemon doc, seats it as the published face's owner, and announces the self-certifying card onto the deterministic per-Nexus WHO board the relay syncs. It FAILS CLOSED when the daemon doc carries no prefix — a face with no persona to own it never falls back to self-ownership (which forecloses recovery). The testable core (`publishHandleFromDaemonDoc`) is platform-blind; the node adapter (`runHandlePublish`) supplies the store/seed/board shores. Browser publish + the phone/QR summons remain.
