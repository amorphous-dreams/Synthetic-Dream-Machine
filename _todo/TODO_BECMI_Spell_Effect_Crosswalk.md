...
- 2026-03-25 Expert uplift pass (batch 1): implemented context-first note expansion for core Expert ECM rows (`Silence 15' Radius`, both `Dispel Magic` rows, `Anti-Magic Shell`, `Protection from Magic`, `Ring of Spell Turning`) and high-tier arcane procedures (`Cloudkill`, `Death Spell`, `Disintegrate`, `Geas`, `Hallucinatory Terrain`, `Ice Storm/Wall`, `Lower Water`, `Massmorph`, `Projected Image`, `Stone to Flesh`, plus polymorph pair detail locks). Mapping status promotions in this batch: `Cloudkill`, `Death Spell`, `Disintegrate`, `Geas` -> `custom`; `Hallucinatory Terrain`, `Massmorph`, `Projected Image`, `Stone to Flesh` -> `partial`.
- 2026-03-25 Expert uplift pass (batch 2): resolved the remaining high-priority Expert `undecided` rows with source-grounded procedure notes and dual-row consistency updates. Promotions in this batch: `Bless`, both `Continual Light` rows, `Find Traps`, `Growth of Animal`, `Snake Charm`, and `Mirror Image` -> `partial`; both `Remove Curse` rows and `Phantasmal Force` -> `custom`. Also normalized the `Ice Storm/Wall` status cell back to canonical status syntax (`custom`).
- Expert-lane crosswalk uplift (batch 2, 2026-03-25) resolved the remaining high-priority Expert `undecided` rows and added procedure-locked notes for key bridge recognizers (`Continual Light`, `Remove Curse`, `Mirror Image`, `Phantasmal Force`, and related cleric interfaces), so Chapter 06 can now treat Expert recognizers as bridge-ready inputs.
