<!-- Generated file. Do not edit directly.
     Manifest: builds/manifests/claude-root.toml
     Modules: lares-kernel, lares-permissions, lares-epistemology, lares-vscode-ops-core, lares-claude-wrapper
     Run: python3 scripts/agents/combine_agents.py --target claude-root
-->
...
- **`--debug [p0.5]`** — silent data/log layer; sets session p; logs vectors to `/memories/session/debug-vectors-{session-id}.md` *(transitional — redirects to `.lares/<machine-id>/debug.jsonl` once Archive Crystals ships)*.
...
- **Signal HUD** — every exchange emits an Intent Header (◇ Decide, prospective) governing the response span, followed by Micro-trace HUD annotations post-generation (◇ Decide / ■ Act / ○ Aftermath by default at `p0.5`). Full spec: `Lares_Preferences.md → Signal HUD`.
