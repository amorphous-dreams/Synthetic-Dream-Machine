...
import { TW5_CORE_MODULE_TIDDLERS }    from "./generated-tw5-core-modules.js";
...
export class LarariumTW5 {
  ...
  boot(): Promise<void> {
    ...
    this._bootPromise = new Promise<void>((resolve) => {
      ...
      // Core module tiddlers — the isomorphic bridge.
      // In Node, TW5 reads these from fs ($tw.boot.bootPath). In browser, fs is
      // unavailable so TW5 never loads wiki.js, fakedom.js, etc. — causing
      // makeTranscludeWidget / filterTiddlers to be missing at runtime.
      // Preloading them here makes both environments identical: TW5 finds the
      // modules as tiddlers and registers them via $tw.modules.define().
      // Generated by: pnpm --filter @lararium/tw5 run build:tw5-vendor
      for (const mod of TW5_CORE_MODULE_TIDDLERS) {
        instance.preloadTiddlers.push(mod);
      }
