...
| S8.1 | Canon-promote hardening + ahu render rewrite (E.10.1→E.10.4) | ✅ Complete | Six interlocking bugs in cross-bag promote ceremony fixed; `lares draft` ceremony lands; canonical bags open as writable+defaultWritable:false; AhuWidget rewritten as TW5-native cascade-resolve + transclude shim with template tiddlers `lar:///ha.ka.ba/@lararium/templates/ahu/{html,markdown-meme}`; new wikirule `wikirules/memetic-wikitext-sigil.ts` makes `<<~` first-class TW5 grammar (block + inline forms); MemeticParser+deserializer registration synchronous (race fix). Round-trip verified end-to-end. |
...
| H | Save-side auto-split (TW5 UX → child tiddlers) | ⬜ Next | `MemeSyncAdaptor.saveTiddler` runs `parseMemeText` → `splitMemeToTiddlers` on memetic-wikitext content; writes parent + children; tombstones removed children. Fourth call site of the same parser, symmetric with disk sync. |
| I | Wikifier DOCTYPE + dash + macrocall recovery | ⬜ Polish | Three small diff items in current round-trip output: HTML comment DOCTYPE dropped, `|---|` → `|—|`, aka URI sigil routes through ahu (empty result). Aesthetic, not load-bearing. |
