...
| I | Wikifier polish (DOCTYPE + dash) | ⬜ Single-backtick cured by V.2; DOCTYPE/dash remain | Parser instantiation now canonical; remaining DOCTYPE comment + `dash` rule conversions are aesthetic round-trip diffs in the rendering pipeline. Defer until G+H ship. |
