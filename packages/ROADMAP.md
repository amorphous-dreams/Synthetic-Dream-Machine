...
| V.2 | Boot-path conversion to plugin loader | ⬜ Next | Replace imperative `_registerWidgets` + `parsers["text/x-memetic-wikitext"] = ...` mutations in `tw5-vm.ts` with plugin-tiddler load via TW5's standard `$tw.modules` flow. **Side effect: fixes single-backtick parser regression.** Engine corpus loads the plugin tiddler at boot. |
| W | CodeMirror 6 / Lezer alignment + Memetic Wikitext LSP server | ⬜ Designed (way downstream — gated on CLI wiki stability) | Two-stage: (1) wikirule patterns map onto CM6 + Lezer grammar; `tobibeer/codemirror-6-tw5` is the closest active community effort. Adds `module-type: editor` to the plugin for visual editing of memetic-wikitext. linonetwo's `Modern.TiddlyDev` Vite harness is the natural development surface. (2) **Operator's TW5-Grammar textmate VSCode plugin evolves into a full Memetic Wikitext LSP server** — Lezer grammar definitions feed both CM6 (browser TW5 + standalone) and the LSP (VSCode + JetBrains + neovim + any LSP-aware editor). Single grammar, three editor surfaces (TW5 wiki, browser CM6, IDE LSP). Operator-stated trigger: "once CLI wikis stabilize completely." |
| G | Other sigils via wikirule + cascade + templates | ⬜ Next | Port aka, kahea, kau, lele, papalohe, pranala, pae to the same template-cascade architecture ahu now uses. Each sigil may have block and/or inline modes. |
...
| I | Wikifier polish (DOCTYPE + dash) | ⬜ Will resolve via V.2 | Single-backtick + DOCTYPE diffs trace to parser instantiation; V.2's plugin-loader path uses TW5's proper construction sequence. |
| J | Per-slot iam emission (default-elision) + preamble/postamble fields | ⬜ Polish | Operator's idempotency rule: emit only operator-authored fields; child slots elide values matching parent's iam defaults. New tiddler fields `preamble` / `postamble` for non-sigil text around body sigils (streaming-text terminology). |
