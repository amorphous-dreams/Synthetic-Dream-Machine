 ...
 * The defaults track the `pnpm --filter @lararium/node lararium:init` shape so
 * `lares init` lands the same artifacts in the same places.
 */
function defaultDirs(): { storageDir: string; genesisDir: string } {
  // From src/commands/init.ts → ../../  reaches the lararium-node package root.
