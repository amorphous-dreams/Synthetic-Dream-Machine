...
function earliest(...hits) {
    ...
    for (const { tag, h } of hits) {
        if (!h)
            continue;
        if (!best || h.index < best.index)
            best = { tag, index: h.index, end: h.end, cap: h.cap };
    }
    return best;
}
export class MemeStreamParser {
    _buf = "";
    _pos = 0; // parse cursor (absolute index into _buf)
    _state = "idle";
    _uri = "";
    _sohStart = 0; // absolute index of SOH match start — for fullText reconstruction
    // ahu accumulation (inside body)
    _inAhu = false;
    _ahuSlot = "";
    _ahuStart = 0; // absolute index just after <<~ ahu >> open sigil
    _ahuDepth = 0;
    ...
    _drain() {
        ...
        while (safety++ < 10000) {
            ...
            if (this._state === "body") {
                if (!this._inAhu) {
                    const hit = earliest({ tag: "ahu", h: find(remaining, AHU_OPEN_RE) }, { tag: "etx", h: find(remaining, ETX_RE) }, { tag: "eot", h: find(remaining, EOT_RE) });
                    if (!hit)
                        break;
                    if (hit.tag === "ahu") {
...
//# sourceMappingURL=meme-stream.js.map