 ...
 * validatePranalaEdge() enforces per-family property contracts before edges
 * enter the boot closure.
 */

import { validatePranalaEdge, type PranalaEdge } from "../src/pranala-parser.js";
...
describe("validatePranalaEdge — family contracts", () => {
  ...
  test("control without role → warning, not error", () => {
    const v = validatePranalaEdge(edge({ family: "control", role: null }));
    expect(v).toHaveLength(1);
