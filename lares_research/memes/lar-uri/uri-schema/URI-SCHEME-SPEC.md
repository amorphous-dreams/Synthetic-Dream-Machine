...
- **HUD form compliance:** `lar:` URIs in HUD / render-target surfaces diverge from strict RFC 3986 (emoji substitution). These are *render targets*, not canonical URIs. See companion `URI-OPERATIONS.md` §6.
...
Record form: `stances=^.?^-` (URL-safe characters only, no percent-encoding needed). Render targets map `^` → `+` for display (see `URI-OPERATIONS.md` §6).
...
HUD rendering of phase sigils is defined in `URI-OPERATIONS.md` §6.
...
Operational semantics (aftermath integration, progressive disclosure, deferred features) are in `URI-OPERATIONS.md` §5.
...
11. Exchange-closing URIs end with ` → ?`. System file locus openers use `∞ →`; system file closers use `→ ?`. Section-level markers within system files use `ahu` (waypoint) or `kahea` (transclusion) — not span sigils. See `URI-OPERATIONS.md` §4 (Marker Ontology).
...
The rendering table (`URI-OPERATIONS.md` §6.1) governs the canonical-to-render-target transform for HUD lines and post-headers. Render-target surfaces (glyph-rich) are not stored in spanSpan URI fields.
...
Additional prior art relevant to the operational semantics (OODA-HA phase model, OTel trace context, Kowloon export, FFZ protocol) is documented in `URI-OPERATIONS.md` §11.
...
*End of scheme specification. This document defines the grammar of the `lar:` URI scheme. Operational semantics — exchange protocol, render targets, marker ontology, calibration mapping — are in `URI-OPERATIONS.md`.*
