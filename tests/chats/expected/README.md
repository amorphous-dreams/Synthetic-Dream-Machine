...
> Part of the [Archived Lares Behavioral Test Suite](../README.md)  
...
|------|-----------------|--------|-------|
| `C-06--parse-boundary-vs-trace.md` | C-series (CLI Conventions) — parse boundary vs OODA-HA trace split | **green** | Pins dense parse segmentation plus sparse event-level reply trace without conflating the two layers |
...
- `tests/chats/expected/Lares_Test_Prompt_and_Output_Coffee_Oracle.md` ✓
...
2. Identify which probe(s) from `tests/chats/plans/v0.4/probes/` this output best illustrates
